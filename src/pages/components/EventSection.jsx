...
//         "http://localhost:8080/auth/users/organizers",
//         {
//           headers: {
//             Authorization: `Bearer ${localStorage.getItem("token")}`
//           }
...
//     try {
//       await axios.put(
//         `http://localhost:8080/api/events/${assignEventId}/assign-organisers`,
//         selectedOrganisers,
//         {
//           headers: {
//             Authorization: `Bearer ${localStorage.getItem("token")}`
...
//     try {
//       await axios.post(
//         `http://localhost:8080/api/event-config/${selectedEventId}/upi`,
//         { upiId: upi },
//         {
//           headers: {
//             Authorization: `Bearer ${localStorage.getItem("token")}`
...
//     try {
//       await axios.post(
//         `http://localhost:8080/api/admin/events/${categoryEventId}/categories`,
//         categoryForm,
//         {
//           headers: {
//             Authorization: `Bearer ${localStorage.getItem("token")}`
...
//         {events.map((event) => (
//           <div key={event.id} className="shadow-lg p-4 rounded-xl bg-white">
...
//               <button
//                 onClick={() => openAssignModal(event.id)}
...
//               <button
//                 onClick={() => openCategoryModal(event.id)}
...
//         >
//           <div
//             className="bg-white p-5 rounded w-[500px] relative"
//             onClick={(e) => e.stopPropagation()}
//           >
//             <button onClick={() => setShowCategoryModal(false)} className="absolute top-2 right-2">
//               <X />
//             </button>
...
//               <button onClick={saveUpi} className="bg-green-600 text-white px-3 py-1">
//                 Save
//               </button>
//             </div>
...
//       {/* ORGANISER MODAL (HEADER + ASSIGNED LABEL) */}
//       {showAssignModal && (
//         <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center">
//           <div className="bg-white p-5 rounded w-96 relative">
...
//             <button
//               onClick={assignOrganisers}
//               className="bg-indigo-600 text-white px-3 py-2 mt-3 w-full"
//             >
//               Assign
//             </button>
...
const EventSection = () => {
  ...
  const saveCoupon = async () => {
    ...
    try {
      if (editingCouponId) {
        await axios.put(
          `http://localhost:8080/api/admin/events/${couponEventId}/coupons/${editingCouponId}`,
          couponForm,
        ...
        await axios.post(
          `http://localhost:8080/api/admin/events/${couponEventId}/coupons`,
          couponForm,
          { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
        );
  ...
  const deleteCoupon = async (id) => {
    try {
      await axios.delete(
        `http://localhost:8080/api/admin/events/${couponEventId}/coupons/${id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`
          }
        ...
        {events.map((event) => (
          <div key={event.id} className="shadow-lg p-4 rounded-xl bg-white">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              ...
              <button onClick={() => openCouponModal(event.id)} className="bg-yellow-600 text-white px-2 py-1 rounded">
                Coupons
              </button>

              <button onClick={() => { setSelectedEventId(event.id); setShowUpiModal(true); }} className="bg-purple-600 text-white px-2 py-1 rounded">
                UPI
              </button>

              <button onClick={() => openAssignModal(event.id)} className="bg-indigo-600 text-white px-2 py-1 rounded">
                Assign Organisers
              </button>

              <button onClick={() => openCategoryModal(event.id)} className="bg-green-600 text-white px-2 py-1 rounded">
                Add Ticket Categories
              </button>
